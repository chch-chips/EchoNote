...
- 2026-07-11：CNB build-only POC、服务器 bootstrap 与首次容器切换完成；生产 Prisma schema 经只读 diff 确认后补齐两条既有 migration 记录，未执行重复 DDL。
- 2026-07-11：PR #8 合并后完成首次 GitHub → CNB → TCR → CVM 自动部署。生产当前运行的 release 由 `/var/lib/echonote-deploy/current.env` 记录；Web 与 nginx 健康检查均返回 `200`，旧 systemd 服务已禁用。
