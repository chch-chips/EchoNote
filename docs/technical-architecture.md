...
- 展示层：首页用 24 小时快照 + Three.js 做记忆雨；历史页提供时间流和搜索。
...
索引：`createdAt` 用于取最近候选池，`weight` 为未来按权重查询或调优预留。
...
接口每次读取时优先找 `refreshAfter > now` 的最新快照。如果快照里的片段都还存在，就复用它；如果片段缺失或快照过期，就重新生成。系统只保留最近 12 个快照，避免表无限增长。
...
```json
{
  ...
  "refreshAfter": "...",
  "fragments": [
    { "id": "...", "text": "...", "tone": "...", "weight": 2, "createdAt": "..." }
  ]
}
```

如果还没有 AI 片段，则从最近小记中截取内容作为降级展示。
...
`src/components/memory-rain.tsx` 是客户端组件，因为 Three.js 依赖 DOM 和 Canvas。它只负责渲染，不负责决定“今天显示哪批记忆”；这件事由 `/api/memory-rain` 和 `MemoryRainSnapshot` 完成。
...
4. 如果片段为空但快照仍有效，返回同一个 `seed`，片段从最近 `Note` 截断降级生成，确保空库时页面也有内容。
5. 如果没有有效快照，或快照片段已经不完整，则生成新快照。

生成快照时：

1. 读取最近 `MEMORY_RAIN_FRAGMENT_LIMIT * 3` 条 `MemoryFragment` 作为候选池。
2. 用 `randomInt(1, 2147483647)` 生成 seed。
3. 用同一个 seed 初始化线性同余随机数发生器。
4. 对候选片段执行加权抽样：`key = -ln(u) / weight`，key 越小越靠前。这样 `weight` 越高，片段越容易进入快照，但仍保留随机性。
5. 最多保留 `MEMORY_RAIN_FRAGMENT_LIMIT` 条，默认 80，代码硬上限 160。
6. 写入 `MemoryRainSnapshot.fragmentIds` 和 `refreshAfter`。
7. 删除第 13 个之后的旧快照，只保留最近 12 个。

如果还没有任何 `MemoryFragment`，后端从最近 40 条 `Note` 生成 fallback 片段：`text=note.content.slice(0, 120)`，`tone=null`，`weight=1`。
...
type MemoryRainResponse = {
  seed?: number;
  fragments?: { id: string; text: string; weight?: number }[];
};
```
...
2. 用后端返回的 `seed` 初始化同样的线性同余随机数发生器。
3. 创建 Three.js `Scene`、`PerspectiveCamera` 和透明 `WebGLRenderer`。
4. 每条文字先画到 720x128 的 Canvas，再转成 `THREE.CanvasTexture`。
5. 用 texture 创建 `THREE.SpriteMaterial`，再创建 `THREE.Sprite`。
6. 用 seeded random 生成每个 sprite 的初始位置、scale、opacity、speed、phase。
7. sprite 数量控制在 12 到 48 之间：片段少时循环复用，片段多时限制数量，避免背景太重。

因为位置和参数都来自同一个 seed，所以 24 小时内反复打开页面，记忆雨的初始构成是稳定的；动画仍然持续流动，所以不会显得死板。

#### 前端如何渲染和刷新

动画循环里每帧会：

1. 让每个 sprite 按自己的 `speed` 向下移动。
2. 用 `Math.sin(Date.now() * 0.00025 + phase + index)` 给 x 方向一点轻微漂移。
3. y 小于 `-520` 时回到 `520`，形成循环下落。
4. 让整个 group 以很小幅度绕 y 轴摆动。
5. 调用 `renderer.render(scene, camera)` 并继续 `requestAnimationFrame`。
...
如果用户系统设置 `prefers-reduced-motion: reduce`，组件不会启动 Three.js 动画。组件卸载时会移除 resize listener，释放 texture/material/renderer，并清空宿主节点，避免热更新或路由切换后残留 WebGL 资源。

移动端通过 `max-sm:opacity-35` 降低记忆雨存在感，确保输入面板仍然是第一优先级。
