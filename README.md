...
- 私有部署：当前稳定版本由腾讯云服务器上的 systemd 守护；新发布链路正在迁移为 GitHub CI + CNB 云端构建 + TCR + Docker Compose。
...
CNB 免费接入、服务器 bootstrap、首次 POC 和回滚演练见 [CNB 部署接入](docs/cnb-setup.md)。在 POC 与回滚演练完成前，生产继续沿用现有 systemd 服务。
