...
- 首页用交互式 Three.js 呈现空间化“记忆雨”，点击落下文字可取回对应小记原文。
...
本地或 dev 数据库需要应用 schema 变更时使用 Prisma migration：
...
项目使用 PostgreSQL。当前采用最小企业级 local-dev 方案：本机运行 Web，连接服务器 PostgreSQL 容器里的隔离 dev 数据库。

本地开发数据库：

- 数据库：`echo_note_dev`
- 用户：`echo_note_dev_user`
- 隧道：`127.0.0.1:15432`
...
- 数据库：`echo_note`
- 用户：`echo_note_user`
- 服务：`echonote-web` / `echonote-worker`

上线后 schema 变更以 `prisma/migrations/` 为准：本地/dev 使用 `npm run db:migrate` 或 `npm run db:deploy` 应用迁移，生产发布使用 `npm run db:deploy`。不要把 `prisma db push` 作为常规生产流程。

当前机器不需要本地安装 PostgreSQL。通过 SSH 隧道连接服务器数据库端口：

```powershell
ssh -N -L 15432:127.0.0.1:5432 chips-server
```

然后在 `.env` 中把 `DATABASE_URL` 指向 `echo_note_dev`。不要把 PostgreSQL 5432 端口暴露到公网，也不要让本地 `.env` 指向生产库。
...
AI 分析、记忆碎片、24 小时记忆雨快照和 Three.js 交互渲染链路见 [技术架构说明](docs/technical-architecture.md)。
